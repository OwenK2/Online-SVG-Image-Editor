var mode = "cursor";
var boards = [], b = -1, editing = [], activeEdit = [], zoom = 1;
var mx = 0, my = 0, sx = 0, sy = 0, rx = 0, ry = 0, ax = -50, ay = -50, keys = {},down = false, resizing = false, moving = false, fromMenu = true;
var editor, activeElement, dragColor, d, dragging;
var pathConnector;
var defaults = {
  tag: 'default',
  fill: "#000000",
...
function load() {
  ...
  document.getElementById("artspace").addEventListener("mousewheel", function(e) {if(b !== -1) {scaleZoom(e);}if(event.ctrlKey) {event.preventDefault();}});
  window.addEventListener("mousemove", mouseMove);
  window.addEventListener("keyup", function(event) {delete keys[event.keyCode];keys.shift = event.shiftKey;keys.alt = event.altKey;if(event.keyCode == 32){cursor();}});
  window.addEventListener("keydown", function(event) {
    ...
    if(b !== -1) {
      ...
      if(event.ctrlKey && keys[187] || event.metaKey && keys[187]) {event.preventDefault();scaleZoom(undefined,1)} //Zoom In
      if(event.ctrlKey && keys[189] || event.metaKey && keys[189]) {event.preventDefault();scaleZoom(undefined,-1);} //Zoom Out
      if(keys[46] && editing.length > 0 && document.activeElement === document.body) {for(var i=0;i<editing.length;i++) {editing[i].remove();};editing = [];}
  ...
  colorInitalize();
  updateOptions();
  new Artboard(50, 50, 100, 100, 'Test');
  boards[b].center();
  closePopup();
  resize();
}
...
function setMode(md) { //SET MODE OF EDITOR
  ...
  for(var i = 0;i<boards.length;i++) {
    if(boards[i]) {boards[i].blurElements();}
  }
  cursor();
  updateOptions();
}
var noRender = ['x', 'y', 'w', 'h', 'x1', 'y1', 'x2', 'y2','rotation']
function setOption(elem) {
  var target = editing.length > 0 ? editing : [defaults];
  var parts = elem.id.split("_");
  for(var i = 0;i<target.length;i++) {
    if(parts[1] == "x1" && !isNaN(parseFloat(elem.value))) {target[i].horAlignment = null;target[i].start.x = parseFloat(elem.value);target[i].start.draw();}
    else if(parts[1] == "y1" && !isNaN(parseFloat(elem.value))) {target[i].vertAlignment = null;target[i].start.y =  parseFloat(elem.value);target[i].start.draw();}
    else if(parts[1] == "x2" && !isNaN(parseFloat(elem.value))) {target[i].horAlignment = null;target[i].end.x = parseFloat(elem.value);target[i].end.draw();}
    else if(parts[1] == "y2" && !isNaN(parseFloat(elem.value))) {target[i].vertAlignment = null;target[i].end.y = parseFloat(elem.value);target[i].end.draw();}
    else if(parts[1] == 'font') {target[i].font = elem.value;}
    else if(elem.type === "text" || elem.type === "number") {
      var points = target[i].points ? target[i].points : target[i].start ? [target[i].start,target[i].end] : target[i];
      if(parts[1] == 'x') {target[i].horAlignment = null;alignElem(target[i].board,points,target[i],parseFloat(elem.value),null);}
      else if (parts[1] == 'y') {target[i].vertAlignment = null;alignElem(boards[b],points,target[i],null,parseFloat(elem.value));}
      else if (parts[1] == 'rotation' && !isNaN(parseFloat(elem.value))) {target[i].rotation = parseFloat(elem.value);}
      else if(parts[1] == 'w' && target[i] instanceof Text) {target[i].explicitWidth = true;target[i].w = elem.value;}
      else {target[i][parts[1]] = elem.value;}
    }
      ...
      if(parts[1] === "prev") {
        ...
        if(parts[2] === 'stroke') {target[i].doStroke = true;}
        if(parts[2] === 'fill') {target[i].doFill = true;}
      }
      target[i][parts[1]] = elem.checked;
    }
    setRadio("vertAlignment",target[i].vertAlignment,target[i] === defaults);
    setRadio("horAlignment",target[i].horAlignment,target[i] === defaults);
  }
  if(editing.length > 0) {
    for(var i=0;i<editing.length;i++) {
      editing[i].render(noRender.indexOf(parts[1]) > -1);
      if(editing[i] instanceof Text && mode === 'text' && editing.length == 1 && noRender.indexOf(parts[1]) === -1) {
        editing[i].editText();
      }
    }
  }
...
function Artboard(x,y,w,h,title) { //CREATES an ARTBOARD
  ...
  this.label.className = "artLabel";
  this.label.onmousedown = function() {
    this.style.cursor = "grabbing";
    dragging = self;
  }
  this.e.appendChild(this.label);
  this.elements = [];
  this.markers = [];
  this.h = h;
  this.w = w;
  ...
  this.render = function() { //UPDATES VISUALS & EDITOR CODE
    ...
    this.e.style.height = this.h * zoom + "px";
    this.e.style.width = this.w * zoom + "px";
    this.label.textContent = this.title + ".svg";
    this.label.style.fontSize = Math.max(this.w * zoom/20,10) + "px";
    ...
    document.getElementById("svg_"+this.id).onmousedown = function(event) { //WHEN YOU CLICK ON AN ELEMENT
      ...
      if(editing.indexOf(boards[b].elements[id]) === -1) {
        if(mode === 'cursor' && id < boards[b].elements.length && !keys[32]) {
        ...
        else if(mode === 'text' && id < boards[b].elements.length && boards[b].elements[id] instanceof Text && !keys[32]) {
          if(!event.ctrlKey) {
            boards[b].blurElements();
            editing = [boards[b].elements[id]];
          }
          ...
          boards[b].elements[id].editText();
          updateOptions();
        }
        else if(mode === 'text' && event.path.indexOf(document.getElementById("artspace")) > -1 && !keys[32] && !activeElement && editing.length === 0) {
          activeElement = new Text(boards[b],rx/zoom,ry/zoom);
        }
        else {
          boards[b].blurElements();
        }
    ...
    for(var i=0;i<this.markers.length;i++) {
      if(this.markers[i] && this.markers[i].draw) {this.markers[i].draw();}
    }
    for(var i=0;i<this.elements.length;i++) {
      if(this.elements[i] && this.elements[i].setEvent) {this.elements[i].setEvent();}
    }
    editor.setValue(format(document.getElementById("svg_"+this.id).outerHTML.replace(/id=("|'|`)[^"'`]+("|'|`)\s?/gm, '')) || '<!-- SVG CODE GOES HERE -->',-1);
    editor.resize();
  };
  this.center = function() { //CENTERS ARTSPACE AND FULLSCREENS IT
    document.getElementById("artWrap").style.transform = "translate("+-this.x+"%,"+-this.y+"%)";
    zoom = Math.min((document.getElementById("artspace").offsetHeight-70)/this.h,(document.getElementById("artspace").offsetWidth-70)/this.w);
    setTransform();
    scaleZoom(zoom);
  };
  ...
  this.findSpace = function() {
    var clear = 0;
    while(clear < 20 && clear !== -1) {
      clear ++;
      var found = false;
      for(var i = 0;i<boards.length;i++) {
        if(boards[i] && boards[i] !== this) {
          var box = boards[i].e.getBoundingClientRect();
          var abox = this.e.getBoundingClientRect();
          if(!(box.left > abox.right||box.right < abox.left||box.top > abox.bottom||box.bottom < abox.top)) {
            this.y = boards[i].y;
            this.x = boards[i].x + 1.5 + (boards[i].e.offsetWidth/2 + this.e.offsetWidth/2)/document.getElementById("artWrap").offsetWidth*100;
            this.render();
            var found = true;
          }
        }
  ...
  this.blurElements = function(elem) {
    if(elem && editing.indexOf(elem) > -1) {
      ...
      for(var i = 0;i<markers.length;i++) {
        if(markers[i].marker) {markers[i].marker.style.display = "none";if(markers[i].cmarker) {markers[i].cmarker.style.display = "none";}}
        else {markers[i].style.display = "none";}
      } 
      if(elem instanceof Text) {activeElement = null;}
    }
    else {
      if(editing.length > 0) {
        editing = [];
        ...
        for(var i = 0; i < this.markers.length; i++) {
          if(this.markers[i]) {
            ...
            if(this.markers[i].cmarker) {this.markers[i].cmarker.style.display = "none";}
          }
        }
      }
    }
  ...
  this.remove = function() { //DELETES ARTBOARD
    ...
    boards[this.id] = null;
    if(document.getElementById("artWrap").children.length === 0) {
      ...
      while(!boards[b+i]) {
        i --;
        if(b+i < 0) {i = boards.length-1 - b;}
        if(boards[b+i]) {boards[b+i].focus();break;}
      }
    }
  }
...
function Point(master, x, y, cx,cy) { //CREATES POINT
  ...
  this.editingcpoint = false;
  this.marker = document.createElement("div");
  this.marker.id = "point_"+this.board.id + '_' + this.id;
  this.marker.className = "marker";var self = this;
  if(cx && cy) {this.makeCpoint(cx,cy);}
  this.marker.onmousedown = function() {
    if(keys.alt && !self.cmarker) {
      if(self.makeCpoint(self.x,self.y)) {
        self.editingcpoint = true;
        self.cmarker.style.display = "block";
        self.editingcpoint = true;
        activeEdit = [self];
        prepMove(activeEdit);
        editCurrent();
    ...
    self.editingcpoint = false;
    activeEdit = [self];
    prepMove(activeEdit);
    editCurrent();
  }
  this.board.e.appendChild(this.marker);
  this.zoom = function(factor) { //SCALES COMPONENTS
    this.x /= factor;
    this.y /= factor;
    if(this.cmarker) {
      this.ctrlX /= factor;
  ...
  this.translate = function(x,y) { //Translates Components
    this.x = this.ox*zoom + x - rsx*zoom;
    this.y = this.oy*zoom + y - rsy*zoom;
    if(this.cmarker) {
      this.ctrlX = this.cox*zoom + x - rsx*zoom;
      this.ctrlY = this.coy*zoom + y - rsy*zoom;
    }
    this.zoom(zoom);
    this.draw();
  }
  this.makeCpoint = function(x,y)  {
    if(this.master instanceof Path) {
      if(this.cmarker) {this.board.e.removeChild(this.cmarker);this.cmarker = undefined;}
      this.ctrlX = this.cox = x;
      this.ctrlY = this.coy = y;
      this.cmarker = document.createElement("div");
      this.cmarker.id = "cpoint_"+this.board.id + '_' + this.id;
      this.cmarker.className = "marker cpoint";var self = this;
      this.cmarker.onmousedown = function() {
        if(keys.alt) {
          self.ctrlX = self.cox = undefined;
          self.ctrlY = self.coy = undefined;
          self.board.e.removeChild(self.cmarker);
          self.cmarker = undefined;
          self.master.render();
          return false;
        }
        self.editingcpoint = true;
      ...
      this.board.e.appendChild(this.cmarker);
      return true;
    }
    else {return false;}
  }
  this.draw = function(dontDo) {
    ...
    if(this.cmarker) {
      this.cmarker.style.height = this.cmarker.style.width = this.r + "px";
      this.cmarker.style.left = this.ctrlX / this.board.w * 100 + "%";
      this.cmarker.style.top = this.ctrlY / this.board.h * 100 + "%";
    }
    if(this.master instanceof Rect && this.master.points && !dontDo || this.master instanceof Ellipse && this.master.points && !dontDo) {
      var diagonal = this.master.points.indexOf(this) + 2;
      if(diagonal > 3) {diagonal -= 4;}
      for(var i = 0;i<this.master.points.length;i++) {
        if(this.master.points[i] !== this && i !== diagonal) {
          var prev = i > 0 ? this.master.points[i-1] : this.master.points[3];
          var next = i < 3 ? this.master.points[i+1] : this.master.points[0];
          if((i+1) % 2 === 0) {
            this.master.points[i].x = next.x;
            this.master.points[i].y = prev.y;
          }
          else {
            this.master.points[i].x = prev.x;
            this.master.points[i].y = next.y;
          }
          this.master.points[i].draw(true);
        }
      }
    }
  };
  this.edit = function(cpoint) { //EDITS DISPLAY POINT & CHORDS
    if(cpoint) {
      this.ctrlX = this.cox + rx/zoom - rsx;
      this.ctrlY = this.coy + ry/zoom - rsy;
    }
    else {
      this.x = this.ox + rx/zoom - rsx;
      this.y = this.oy + ry/zoom - rsy;
    }
     if(keys['shift'] && cpoint) {
      var other = this.master.points[this.master.points.indexOf(this)-1] || this.master.points[this.master.points.length-1];
      var midpoint = {x:(this.x + other.x)/2,y:(this.y + other.y)/2};
      var dist = Math.sqrt((this.x-other.x)*(this.x-other.x)+(this.y-other.y)*(this.y-other.y))/2;
      var angle = Math.atan2(this.y-other.y,this.x-other.x);
      var side = (this.ctrlX-other.x)*(this.y-other.y)-(this.ctrlY-other.y)*(this.x-other.x);
      var factor = side < 0 ? Math.PI/2 : -Math.PI/2;
      this.ctrlX = midpoint.x+dist*Math.cos(angle+factor);
      this.ctrlY = midpoint.y+dist*Math.sin(angle+factor);
    }
    else if(keys['shift']) {
      if(this.master instanceof Rect || this.master instanceof Ellipse) {
        var i = this.master.points.indexOf(this);
        var diagonal = i>1 ? this.master.points[i - 2] : this.master.points[i + 2];
        var mag = Math.max(Math.abs(this.x - diagonal.x),Math.abs(this.y - diagonal.y));
        this.x = this.x - diagonal.x > 0 ? diagonal.x + mag : diagonal.x - mag;
        this.y = this.y - diagonal.y > 0 ? diagonal.y + mag : diagonal.y - mag;
      }
      else {
    ...
    this.master.horAlignment = null;
    this.master.vertAlignment = null;
    this.draw();
    this.master.render();
    updateOptions();
  }
  this.remove = function() {
    this.board.markers[this.id] = null;
    if(this.cmarker) {this.board.e.removeChild(this.cmarker);}
...
function Group(board,elements) {
  ...
  this.render = function(noupdate) {
    this.box = document.getElementById(this.elem.id).getBBox();
    alignElem(this.board,this,this,this.horAlignment,this.vertAlignment);
    ...
    if(this.rotation !== 0 && !isNaN(this.rotation) && this.rotation !== "") {document.getElementById(this.elem.id).setAttribute("transform", 'rotate('+this.rotation+' ' + (this.x+this.w/2) + ' ' + (this.y+this.h/2) + ')');}
    else {document.getElementById(this.elem.id).removeAttribute("transform");}
  ...
  this.remove = function() {
    this.board.elements[this.id] = null;
    this.board.e.removeChild(this.marker);
...
function Line(board,x,y) { //CREATES NEW SVG LINE
  ...
  this.render = function(noupdate) { //UPDATES VISUALS OF LINE
    alignElem(this.board,[this.start,this.end],this);
    ...
    editor.setValue(format(document.getElementById("svg_"+this.board.id).outerHTML.replace(/id=("|'|`)[^"'`]+("|'|`)\s?/gm, '')) || '<!-- SVG CODE GOES HERE -->',-1);
    if(!noupdate) {
      var box = document.getElementById(this.elem.id).getBBox();
      this.x = box.x;
      this.y = box.y;
      this.h = box.height;
      this.w = box.width;
...
function Path(board,x,y) { //CREATES NEW SVG LINE
  ...
  this.getD = function() {
    ...
    for(var i = 1;i<this.points.length;i++) {
      if(this.points[i].cmarker) {
        d+= " Q " + this.points[i].ctrlX + " " + this.points[i].ctrlY + " " + this.points[i].x + " " + this.points[i].y;
      }
      else {
    ...
    if(this.points[0].cmarker) {
        d+= " Q " + this.points[0].ctrlX + " " + this.points[0].ctrlY + " " + this.points[0].x + " " + this.points[0].y;
    }
    else if(this.ended || this.touchingEnd) {
      d += " z";
    }
    return d;
  }
  this.addPoint = function(x,y,noscale) {
    if(this.touchingEnd && this.points.length > 1) {
      ...
      this.newPoint.ox = rx/zoom;
      this.newPoint.oy = ry/zoom;
      if(noscale) {p.scale(1/zoom);}
      this.points.push(p);
    }
  }
  this.toSvg = function() {
  ...
  this.render = function(noupdate) { //UPDATES VISUALS OF LINE
    ...
    alignElem(this.board,this.points,this);
    if(this.rotation !== 0 && !isNaN(this.rotation) && this.rotation !== "") {document.getElementById(this.elem.id).setAttribute("transform", 'rotate('+this.rotation+' ' + (this.x+this.w/2) + ' ' + (this.y+this.h/2) + ')');}
    else {document.getElementById(this.elem.id).removeAttribute("transform");}
    document.getElementById(this.elem.id).setAttribute("stroke-width", this.strokeWidth);
  ...
  this.onedit = function() { //ON CLICK
    for(var i = 0;i<this.points.length;i++) {
      ...
      if(this.points[i].cmarker) {this.points[i].cmarker.style.display = "block";}
    }
    this.render();
  };
  this.edit = function() {
  ...
  this.remove = function() {
    ...
    if(this.newPoint) {this.newPoint.remove();}
    document.getElementById("svg_"+this.board.id).removeChild(document.getElementById(this.elem.id));
    this.board.code = document.getElementById("svg_"+this.board.id).innerHTML;
    this.board.render();
  }
}
function Text(board,x,y) {
  this.tag = "text";
  this.id= board.elements.length;
  this.x = this.ox = x;
  this.y = this.oy = y;
  ...
  this.underline = defaults.underline;
  ...
  document.getElementById("svg_"+this.board.id).appendChild(this.elem);
  this.marker = document.createElement("div");
  this.marker.id = "marker_"+this.id;
  this.marker.textContent = this.text;
  this.marker.setAttribute('contentEditable', 'true');
  this.marker.className = "textMarker";var self = this;
  this.marker.oninput = function() {
    self.text = this.textContent;
    self.render();
  }
  this.marker.onblur = function() {
    self.marker.style.display = "none";
    activeElement = null;
    if(self.text.replace(/\\s\\r\\n/gi) === '') {self.remove();}
  }
  this.board.e.appendChild(this.marker);
  this.draw = function() {
    this.marker.style.left = this.x*zoom-1 + "px";
    this.marker.style.top = this.y*zoom-this.h*zoom*.15 + "px";
    this.marker.style.fontFamily = this.font;
    if(this.rotation !== 0 && !isNaN(this.rotation) && this.rotation !== "") {
      this.marker.style.transform = "rotate("+this.rotation+"deg)";
    }
    this.marker.style.fontSize = this.fontSize*zoom  + "px";
    this.marker.style.fontWeight = this.bold ? 'bold' : 'normal';
    this.marker.style.fontStyle = this.italic ? 'italic' : 'normal';
    this.marker.style.textAlign = this.align;
    this.marker.style.width = this.w*zoom+2 + "px";
    this.marker.style.height = this.h*zoom + "px";
    this.marker.style.color = this.fill;
    this.marker.style.lineHeight = this.h*zoom + "px";
  }
  this.render = function(noupdate,noalign) {
    if(!noalign) {alignElem(this.board,this,this);}
    var box = document.getElementById(this.elem.id).getBBox();
    if(this.align === 'left') {
      document.getElementById(this.elem.id).setAttribute("text-anchor", 'start');
      document.getElementById(this.elem.id).setAttribute("x", this.x);
      document.getElementById(this.elem.id).setAttribute("y", this.y);
    }
    else if(this.align === 'center') {
      document.getElementById(this.elem.id).setAttribute("text-anchor", 'middle');
      document.getElementById(this.elem.id).setAttribute("x", this.x+this.w/2);
      document.getElementById(this.elem.id).setAttribute("y", this.y);
    }
    else {
      document.getElementById(this.elem.id).setAttribute("text-anchor", 'end');
      document.getElementById(this.elem.id).setAttribute("x", this.x+this.w);
      document.getElementById(this.elem.id).setAttribute("y", this.y);
    }
    document.getElementById(this.elem.id).setAttribute("font-family", this.font);
    document.getElementById(this.elem.id).setAttribute("alignment-baseline", 'hanging');
    document.getElementById(this.elem.id).setAttribute("font-size", this.fontSize);
    if(this.bold) {document.getElementById(this.elem.id).setAttribute("font-weight", 'bold');}
    else {document.getElementById(this.elem.id).removeAttribute("font-weight");}
    if(this.underline) {document.getElementById(this.elem.id).setAttribute("text-decoration", 'underline');}
    else {document.getElementById(this.elem.id).removeAttribute("text-decoration");}
    if(this.italic) {document.getElementById(this.elem.id).setAttribute("font-style", 'italic');}
    else {document.getElementById(this.elem.id).removeAttribute("font-style");}
    document.getElementById(this.elem.id).setAttribute("fill", this.fill);
    if(this.rotation !== 0 && !isNaN(this.rotation) && this.rotation !== "") {document.getElementById(this.elem.id).setAttribute("transform", 'rotate('+this.rotation+' ' + (this.x+this.w/2) + ' ' + (this.y+this.h/2) + ')');}
    else {document.getElementById(this.elem.id).removeAttribute("transform");}
    document.getElementById(this.elem.id).setAttribute("xml:space", 'preserve');
    document.getElementById(this.elem.id).textContent = this.text;
    this.draw();
    this.board.code = document.getElementById("svg_"+this.board.id).innerHTML;
    document.getElementById("svg_"+this.board.id).innerHTML = this.board.code;
    editor.setValue(format(document.getElementById("svg_"+this.board.id).outerHTML.replace(/id=("|'|`)[^"'`]+("|'|`)\s?/gm, '')) || '<!-- SVG CODE GOES HERE -->',-1);
    if(!noupdate) {
      box = document.getElementById(this.elem.id).getBBox();
      this.h = box.height;
      this.w = this.explicitWidth ? this.w : box.width;
      if(box.width > this.w) {this.explicitWidth = false;}
      updateOptions();
    }
    this.setEvent();
  };
  this.setEvent = function() {
    var self = this;
    document.getElementById(this.elem.id).onmouseup = function() {
      dblclick(document.getElementById(self.elem.id), function() {
        if(activeEdit.indexOf(self) > -1) {
          self.editText();
        }
      });
    }
  ...
  this.editText = function() { //ON CLICK
    event.preventDefault();
    this.marker.style.display = "block";
    this.marker.classList.remove("edit");
    this.marker.focus();
    this.render();
  };
  this.onedit = function() {
    this.marker.style.display = "block";
    this.marker.classList.add("edit");
  }
  this.edit = function() {
    if(mode === 'text') {this.editText();return;}
    this.x = this.ox + rx/zoom - rsx;
    this.y = this.oy + ry/zoom - rsy;
    this.render();
  };
...
function Rect(board,x,y) {
  ...
  this.w = null;
  this.h = null;
  this.points = [new Point(this,x,y),new Point(this,x,y),new Point(this,x,y),new Point(this,x,y)]; //Starting with top left going clockwise
  this.rotation = 0;
  this.fill = defaults.fill;
  this.doFill = defaults.doFill;
  this.stroke = defaults.stroke;
  ...
  this.render = function(noupdate) {
    alignElem(this.board,this.points,this);
    var ptBox = pointBox(this);
    document.getElementById(this.elem.id).setAttribute("x", ptBox[0]);
    document.getElementById(this.elem.id).setAttribute("y", ptBox[1]);
    document.getElementById(this.elem.id).setAttribute("width", ptBox[2] - ptBox[0]);
    document.getElementById(this.elem.id).setAttribute("height", ptBox[3] - ptBox[1]);
    if(this.doStroke) {document.getElementById(this.elem.id).setAttribute("stroke", this.stroke);}
    else {document.getElementById(this.elem.id).removeAttribute("stroke");}
    if(this.doFill) {document.getElementById(this.elem.id).setAttribute("fill", this.fill);}
    else {document.getElementById(this.elem.id).setAttribute("fill", 'rgba(0,0,0,0)');}
...
function Ellipse(board,x,y) {
  ...
  this.doStroke = defaults.doStroke;
  this.strokeWidth = defaults.strokeWidth;
  this.elem = document.createElementNS('http://www.w3.org/2000/svg','ellipse')
  this.elem.id = "elem_"+this.board.id+'_'+this.id;
  document.getElementById("svg_"+this.board.id).appendChild(this.elem);
  this.draw = function() {
    this.points[2].edit();
  ...
  this.render = function(noupdate) {
    alignElem(this.board,this.points,this);
    var ptBox = pointBox(this);
    document.getElementById(this.elem.id).setAttribute("cx", (ptBox[0]+ptBox[2])/2);
    document.getElementById(this.elem.id).setAttribute("cy", (ptBox[1]+ptBox[3])/2);
    document.getElementById(this.elem.id).setAttribute("rx", (ptBox[2]-ptBox[0])/2);
    document.getElementById(this.elem.id).setAttribute("ry", (ptBox[3]-ptBox[1])/2);
    if(this.doStroke) {document.getElementById(this.elem.id).setAttribute("stroke", this.stroke);}
    else {document.getElementById(this.elem.id).removeAttribute("stroke");}
    if(this.doFill) {document.getElementById(this.elem.id).setAttribute("fill", this.fill);}
    else {document.getElementById(this.elem.id).setAttribute("fill", 'rgba(0,0,0,0)');}
...
function prepMove(arr) {
  rsx = (mx - boards[b].e.getBoundingClientRect().left)/zoom;
  rsy = (my - boards[b].e.getBoundingClientRect().top)/zoom;
  for(var i =0;i<arr.length;i++) {
    var pts = arr[i].points ? arr[i].points : arr[i].start ? [arr[i].start,arr[i].end] : [arr[i]];
    for(var j = 0;j<pts.length;j++) {
      pts[j].ox = pts[j].x;
      pts[j].oy = pts[j].y;
      if(pts[j].cmarker) {
        pts[j].cox = pts[j].ctrlX;
        pts[j].coy = pts[j].ctrlY;
      }
    }
...
function mouseMove(event) {
  ...
  if(b !== -1) {
    rx = mx - boards[b].e.getBoundingClientRect().left,
    ry = my - boards[b].e.getBoundingClientRect().top;
    if(activeElement && activeElement.draw) {
      activeElement.draw();
    }
  }
  if(keys[32] && down && b !== -1) {
    // document.getElementById("artWrap").style.left = "calc(50% + " + (ax + mx - sx) + "px)";
    // document.getElementById("artWrap").style.top = "calc(50% + " + (ay + my - sy) + "px)";
    var t = "translate(calc("+ax+"% + "+(mx-sx)+"px),calc("+ay+"% + "+(my-sy)+"px))";
    document.getElementById("artWrap").style.transform = t;
  }
  if(resizing) {
  ...
  if(dragging && !keys[32]) {
    var x = dragging.ox + mx - sx;
    var y = dragging.oy + my - sy;
    dragging.x = x/dragging.e.parentNode.offsetWidth*100;
    dragging.y = y/dragging.e.parentNode.offsetHeight*100;
    dragging.render();
  }
  if(moving) {
    var cp = moving.e;
  ...
  if(activeEdit.length > 0) {
    ...
    for(var i =0;i<activeEdit.length;i++) {
      for(var j=0;j<activeEdit[i].board.elements.length;j++) {
        if(activeEdit[i].board.elements[j] && activeEdit[i].board.elements[j].setEvent) {activeEdit[i].board.elements[j].setEvent();}
      }
    }
...
function mouseDown(event) {
  if(!down && b !== -1) {
    ...
    sx = mx, rsx = (mx - boards[b].e.getBoundingClientRect().left)/zoom;
    sy = my, rsy = (my - boards[b].e.getBoundingClientRect().top)/zoom;
    boards[b].ox = boards[b].e.offsetLeft;
    boards[b].oy = boards[b].e.offsetTop;
    if(event.path.indexOf(document.getElementById("artspace")) > -1 && !keys[32] && !activeElement) {
      ...
      if(mode === "rect" ) {activeElement = new Rect(boards[b],rx,ry);}
      if(mode === "ellipse" ) {activeElement = new Ellipse(boards[b],rx,ry);}
    }
    else if(activeElement !== null && activeElement !== undefined && !keys[32] && event.path.indexOf(document.getElementById("artspace")) > -1) {
      if(mode === 'path') {
          activeElement.addPoint(activeElement.newPoint.x*zoom,activeElement.newPoint.y*zoom);
          if(activeElement) {
            activeElement.draw();
            if(event.which === 3) {
              activeElement.toSvg();
...
function mouseUp(event) {
  ...
  activeEdit = [];
  if(dragging) {
    dragging.e.getElementsByClassName("artLabel")[0].style.cursor = "grab";
    dragging = false;
  }
  setTransform();
  ...
  if(activeElement !== null && activeElement !== undefined && !keys[32] && event.path.indexOf(document.getElementById("artspace")) > -1) {
    if(mode === 'line' || mode === 'rect' || mode === 'ellipse') {activeElement.toSvg();activeElement = null;}
  }
}
function scaleZoom(event,delta) {
  mx = event.clientX, my = event.clientY;
  delta = delta = delta*zoom/5 || Math.abs(event.wheelDelta)/event.wheelDelta*zoom/5;
  zoom += delta;
  if(typeof event === 'number') {zoom = event;}
  document.getElementById("artWrap").style.height = 100*zoom+"%";
  document.getElementById("artWrap").style.width = 100*zoom+"%";
  if(zoom < .2) {zoom = .2;}
  if(zoom > 40) {zoom = 40;}
  for(var i =0;i<boards.length;i++) {
    if(boards[i]) {boards[i].render();}
  }
  setTransform();
}
function setTransform() {
  var style = window.getComputedStyle(document.getElementById("artWrap"));
  var matrix = new WebKitCSSMatrix(style.webkitTransform);
  ax = matrix.m41/document.getElementById("artWrap").offsetWidth*100;
  ay = matrix.m42/document.getElementById("artWrap").offsetHeight*100;
}
function editCurrent(val) {
  for(var i = 0;i<activeEdit.length;i++) {
    if(activeEdit[i].edit && activeEdit[i].editingcpoint) {activeEdit[i].edit(true,val);}
    else if(activeEdit[i].edit) {activeEdit[i].edit(false,val);}
  }
}
function newBoard() { //CREATES A NEW BLANK BOARD BASED ON H & W INPUTS
  ...
  var artboard = new Artboard(-ax,-ay, w, h, name);
  artboard.findSpace();
  closePopup();
  resize();
}
...
function updateOptions() {
  ...
  document.getElementById("opt_sec_stroke").style.display = "block";
  document.getElementById("opt_sec_pos").style.display = "none";
  document.getElementById("opt_doFill").disabled = false;
  document.getElementById("opt_doStroke").disabled = false;
  if(target.tag === 'line' || mode === 'line') {
    ...
    document.getElementById("opt_doStroke").checked = true;
    document.getElementById("opt_doStroke").disabled = true;
    ...
    target.rotation === 0 && editing.length === 0 ? document.getElementById("opt_rotation_2").disabled = true : (document.getElementById("opt_rotation_2").disabled = false, document.getElementById("opt_rotation_2").value = target.rotation);
  }
  else if(target.tag === 'path' || mode === 'path') {
    document.getElementById("opt_sec_pos").style.display = "block";
    ...
    target.rotation === 0 && editing.length === 0 ? document.getElementById("opt_rotation").disabled = true : (document.getElementById("opt_rotation").disabled = false, document.getElementById("opt_rotation").value = target.rotation);
  }
  else if(target.tag === 'rect' || mode === 'rect') {
    document.getElementById("opt_sec_pos").style.display = "block";
    target.x === null && editing.length === 0 ? document.getElementById("opt_x").disabled = true : (document.getElementById("opt_x").disabled = false, document.getElementById("opt_x").value = Math.round(target.x*100)/100);
    target.y === null && editing.length === 0 ? document.getElementById("opt_y").disabled = true : (document.getElementById("opt_y").disabled = false, document.getElementById("opt_y").value = Math.round(target.y*100)/100);
    target.w === null && editing.length === 0 ? document.getElementById("opt_w").disabled = true : (document.getElementById("opt_w").disabled = false, document.getElementById("opt_w").value = Math.round(target.w*100)/100);
  ...
  else if(target.tag === 'ellipse' || mode === 'ellipse') {
    document.getElementById("opt_sec_pos").style.display = "block";
    target.x === null && editing.length === 0 ? document.getElementById("opt_x").disabled = true : (document.getElementById("opt_x").disabled = false, document.getElementById("opt_x").value = Math.round(target.x*100)/100);
    target.y === null && editing.length === 0 ? document.getElementById("opt_y").disabled = true : (document.getElementById("opt_y").disabled = false, document.getElementById("opt_y").value = Math.round(target.y*100)/100);
    target.w === null && editing.length === 0 ? document.getElementById("opt_w").disabled = true : (document.getElementById("opt_w").disabled = false, document.getElementById("opt_w").value = Math.round(target.w*100)/100);
  ...
  else if(target.tag === 'text' || mode === 'text') {
    document.getElementById("opt_sec_pos").style.display = "block";
    document.getElementById("opt_sec_stroke").style.display = "none";
    document.getElementById("opt_sec_text").style.display = "block";
    document.getElementById("opt_doFill").checked = true;
    document.getElementById("opt_doFill").disabled = true;
    ...
    target.y === null && editing.length === 0 ? document.getElementById("opt_y").disabled = true : (document.getElementById("opt_y").disabled = false, document.getElementById("opt_y").value = Math.round(target.y*100)/100);
    target.w === null && editing.length === 0 ? document.getElementById("opt_w").disabled = true : (document.getElementById("opt_w").disabled = false, document.getElementById("opt_w").value = Math.round(target.w*100)/100);
    document.getElementById("opt_h").disabled = true;
    document.getElementById("opt_h").value = Math.round(target.h*100)/100;
    setRadio("align", target.align);
  }
  document.getElementById("opt_x").value = Math.round(target.x*100)/100;
  document.getElementById("opt_y").value = Math.round(target.y*100)/100;
  document.getElementById("opt_w").value = Math.round(target.w*100)/100;
  document.getElementById("opt_h").value = Math.round(target.h*100)/100;
  document.getElementById("opt_rotation").value = Math.round(target.rotation*100)/100;
  if(target.tag !== 'line' && mode !== 'line') {document.getElementById("opt_doStroke").checked = target.doStroke;}
  if(target.tag !== 'text' && mode !== 'text') {document.getElementById("opt_doFill").checked = target.doFill;}
...
function cursor() { //HANDLES ANY AND ALL CURSOR CHANGES
  ...
  if(keys[32] && down) {
    ...
    document.getElementById("artspace").style.cursor = "text";
  }
  else {
  ...
  post("php/upload.php", fd, function(data) {
    if(isJson(data)) {
      ...
      Object.keys(files).forEach(function(i,index) {
        var art = new Artboard(50,50,0,0, i.replace('.svg', ''));
        art.intake(files[i]);
        art.focus();
      });
    }
...
function alignElem(board,points,elem,hor,vert) {
  var hor = hor || elem.horAlignment || points.horAlignment;
  var vert = vert || elem.vertAlignment || points.vertAlignment;
  if(points instanceof Text && alignValid(hor) || points instanceof Text && alignValid(vert)) {
    if(hor === "left") {points.x = 0;}
    else if(hor === "center") {points.x = board.w/2 - points.w/2;}
    else if(hor === "right") {points.x = board.w - points.w;}
    else if(typeof hor === 'number') {points.x = hor;}
    if(vert === "top") {points.y = 0}
    else if(vert === "middle") {points.y = board.h/2 - points.h/2;}
    else if(vert === "bottom") {points.y = board.h - points.h*.65;}
    else if(typeof vert === 'number') {points.y = vert;}
    points.render(false,true);
  }
  else if(alignValid(hor) && points || alignValid(vert) && points) {
    var box = boundBox(document.getElementById(elem.elem.id),board);
    for(var i=0;i<points.length;i++) {
      if(hor === "left") {points[i].x -= box[3];points[i].ctrlX -= box[3];}
      else if(hor === "center") {points[i].x += box[4].x;points[i].ctrlX += box[4].x;}
      else if(hor === "right") {points[i].x += box[1];points[i].ctrlX += box[1];}
      else if(typeof hor === 'number') {points[i].x += hor-box[3];points[i].ctrlX += hor-box[3];}
      if(vert === "top") {points[i].y -= box[0];points[i].ctrlY -= box[0];}
      else if(vert === "middle") {points[i].y += box[4].y;points[i].ctrlY += box[4].y;}
      else if(vert === "bottom") {points[i].y += box[2];points[i].ctrlY += box[2];}
      else if(typeof vert === 'number') {points[i].y += vert - box[0];points[i].ctrlY += vert - box[0];}
      points[i].draw(true);
    }
  }
}
function alignValid(val) {
  return typeof val === 'number' && !isNaN(val) || val === "left" || val === "center" || val === "right" || val === "top" || val === "middle" || val === "bottom"; 
}
function boundBox(elem,board) {
  var box = elem.getBBox();
  var center = {x:board.w/2 - (box.x+box.width/2),y:board.h/2 - (box.y+box.height/2)}
  return [box.y,board.w-box.width-box.x,board.h-box.height-box.y,box.x,center];// OFFSET TOP OFFSET RIGHT OFFSET BOTTOM OFFSET LEFT OFFSET CENTER
}
function pointBox(elem) {
  var points = elem.points;
  var box = [Infinity,Infinity,-Infinity,-Infinity]; //left most,top most,right most,bottom most
  for(var i = 0;i<points.length;i++) {
...
var svg = {
  ...
  'rect': `<svg style="fill:transparent !important;" stroke="black" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 100 100" xml:space="preserve"><rect x="5" y="5" width="90" height="90" stroke-width="6" /></svg></div>`,
  'ellipse': `<svg style="fill:transparent !important;" stroke="black" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 100 100" xml:space="preserve"><circle xmlns="http://www.w3.org/2000/svg" r="46" cx="50" cy="50" stroke-width="6"></circle></svg></div>`,
